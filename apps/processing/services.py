...
def calculate_indicators(receivables: Iterable[Receivable], reference_date: date) -> dict[str, object]:
    ...
    overdue_over_five = sum(
        (item.outstanding_amount for item, days in classified if days >= 5),
        Decimal("0"),
    )
    overdue_total = sum(
        (item.outstanding_amount for item, days in classified if days >= 5),
    ...
    return {
        ...
        "overdue_over_five": overdue_over_five,
        "delinquency_percentage": delinquency.quantize(Decimal("0.01")),
        "title_count": len(items),
        "delinquent_customer_count": len({item.customer_id for item, days in classified if days >= 5}),
    }
...
def _top_groups_from_eligible(
    receivables: Iterable[Receivable], limit: int = 10
...
def _top_general_customers_from_eligible(
    receivables: Iterable[Receivable], limit: int = 10
) -> list[dict[str, object]]:
...
def dashboard_rankings(
    receivables: Iterable[Receivable], reference_date: date
) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    """Build both state-dashboard rankings after one eligibility pass."""
    eligible = _eligible_debtor_receivables(receivables, reference_date)
    return _top_groups_from_eligible(eligible), _top_general_customers_from_eligible(eligible)
...
def top_groups(
    ...
    return _top_groups_from_eligible(_eligible_debtor_receivables(receivables, reference_date), limit)
...
def top_general_customers(
    ...
    reference_date = reference_date or date.today()
    return _top_general_customers_from_eligible(_eligible_debtor_receivables(receivables, reference_date), limit)
